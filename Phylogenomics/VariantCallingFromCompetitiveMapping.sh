...
PATHS=XXXXX/Phylogenetics/Mugsy/SspDD04/SanguinisOutgroupAdditionalReferences/CompetitiveMappingSequences

# List of samples to run on
listofsamples=XXXXX/Phylogenetics/SspDD04Samples.txt
...
FAIDX=XXXXX/CompetitiveMapping/JoinedReferences/NewReference/NewReference.fna.fai
...
REF=XXXXX/CompetitiveMapping/JoinedReferences/NewReference/GCF_001578725.1_ASM157872v1_genomic.fna

# bcftools and setGTplugin (allows filtering of variants)
bcftools=XXXXX/Phylogenetics/snpToolkit/bcftools/installed/bin/bcftools
setGTplugin=+XXXXX/Phylogenetics/snpToolkit/bcftools/installed/libexec/bcftools/setGT.so
filltagsplugin=+XXXXX/Phylogenetics/snpToolkit/bcftools/installed/libexec/bcftools/fill-tags.so
...
BAM=XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_mapped_q30_sorted_rmdup_uniq.bam

samtools index $BAM # BAM files need to be indexed
cat $PATHS/SspDD04Regions.txt | tr "\n" " " | xargs samtools view -bh $BAM > XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_filtered_"$species".bam

NEWBAM=XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_filtered_"$species".bam
...
bedtools genomecov -d -ibam $NEWBAM | awk '$3 < 1' | cut -f1,2 > XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_regionsttomask.txt
...
$bcftools mpileup -a AD,DP,SP -Ou -f $REF $NEWBAM | $bcftools call --ploidy 1 -mv |  $bcftools $filltagsplugin -Oz -o XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_calls.vcf.gz -- -t VAF

# Check how many variants there are before filtering (-H does not include header)
VariantsBeforeFiltering=`$bcftools view -H XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_calls.vcf.gz | wc -l`
...
$bcftools view -i 'FORMAT/AD>=3 && QUAL>30 && FORMAT/VAF>=0.90' XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_calls.vcf.gz | $bcftools view -Oz -o XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_filtered.vcf.gz

# Check how many variants there are after filtering
VariantsAfterFiltering=`$bcftools view -H XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_filtered.vcf.gz | wc -l`

# Index VCF
$bcftools index XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_filtered.vcf.gz

# Generate consensus sequence with masked regions
echo "Creating consensus sequence for $Sample"
cat $REF | $bcftools consensus -m XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_regionsttomask.txt XXXXX/CompetitiveMapping/Samples/Sample-"$Sample"/AllSpecies/"$Sample"_SAMandBAMfiles/"$Sample"_"$species"_filtered.vcf.gz > $outdir/"$Sample"_"$species"_competitivemappingconsensus.fa
