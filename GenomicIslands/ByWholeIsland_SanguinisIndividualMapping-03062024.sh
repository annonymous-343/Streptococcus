...
PATHS=XXXX/GenomeAnalysis/GenomicIslands

# Directory for outputs to be made
outdir=$PATHS/ByWholeIsland_SanguinisIndividualMapping-03062024

# Genomic islands input
genomicislands=XXXX/GenomeAnalysis/GenomicIslands/SanguinisGenomicIslands–03062024.txt

# Output of coverage analysis script. Could also do the tsv file with gene info
coveragetable=XXXX/GenomeAnalysis/CoverageAnalysis/Streptococcus_sanguinis/sanguinis-AllSequencesGene-AllGenes_info.txt

# Reference genome for calculating GC content
REF=XXXX/IndividualAlignments/References/Streptococcus_sanguinis/Streptococcus_sanguinis_SK405

# Breadth of coverage data
ancientmappingsummary=XXXX/IndividualAlignments/ResultsSummaries/Streptococcus_sanguinis_MappingResults.txt

modernmappingsummary=XXXX/CompetitiveMapping/Streptococcus_sanguinis_HealthyModernMappingResults.txt

periomodernmappingsummary=XXXX/IndividualAlignments/Streptococcus_sanguinis_PerioModernMappingResults.txt

# Competitive mapping data (for calculating relative abundance)
ancientcompetitivemappingsummary=XXXX/CompetitiveMapping/Samples/AllSpeciesCollectedCompetitiveMappingResults.txt

moderncompetitivemappingsummary=XXXX/CompetitiveMapping/HealthyModernSamples/HealthyModernAllSpeciesCollectedCompetitiveMappingResults.txt

periomoderncompetitivemappingsummary=XXXX/CompetitiveMapping/PerioModernSamples/PerioModernAllSpeciesCollectedCompetitiveMappingResults.txt

# Species name in competitive mapping table
speciesname=S.sanguinis

# Formatted dates (for ancient samples)
metadata=XXXX/GenomeAnalysis/BritishFormattedDates-02292024.txt
...
for Sample in XXXX/CompetitiveMapping/HealthyModernSamples/Sample-*
 do
...
for Sample in XXXX/CompetitiveMapping/PerioModernSamples/Sample-*
 do
...
# Get breadth of coverage data for the islands (single column with name on top)
echo "$NAME" > $outdir/tempbreadth.txt
bedtools coverage -a $outdir/combinedgenomicislandcoordinates.bed -b XXXX/CompetitiveMapping/HealthyModernSamples/Sample-"$NAME"/$COVERAGEFOR/"$NAME"_mapped_q30_sorted_rmdup_uniq.bam | cut -f7 >> $outdir/tempbreadth.txt
...
# Get breadth of coverage data for the islands (single column with name on top)
echo "$NAME" > $outdir/tempbreadth.txt
bedtools coverage -a $outdir/combinedgenomicislandcoordinates.bed -b XXXX/CompetitiveMapping/PerioModernSamples/Sample-"$NAME"/$COVERAGEFOR/"$NAME"_mapped_q30_sorted_rmdup_uniq.bam | cut -f7 >> $outdir/tempbreadth.txt
