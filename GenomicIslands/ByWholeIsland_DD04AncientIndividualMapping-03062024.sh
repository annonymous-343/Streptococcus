...
# Define paths
PATHS=/GenomicIslands

# Directory for outputs to be made
outdir=$PATHS/ByWholeIsland_DD04AncientIndividualMapping-03062024

# Genomic island info
genomicislands=/GenomicIslands/DD04GenomicIslands-03052024.txt


# Output of coverage analysis script. Could also do the tsv file with gene info
coveragetable=/CoverageAnalysis/Streptococcus_spDD04/DD04-AllSequencesGene-AllGenes_info.txt

# Reference genome for calculating GC content
REF=s/IndividualAlignments/References/Streptococcus_spDD04/Streptococcus_sp._DD04

# Breadth of coverage data
mappingsummary=s/IndividualAlignments/ResultsSummaries/Streptococcus_spDD04_MappingResults.txt

# Competitive mapping data (for calculating relative abundance)
competitivemappingsummary=s/CompetitiveMapping/Samples/AllSpeciesCollectedCompetitiveMappingResults.txt

# Species name in competitive mapping table (for calculating relative abundance)
speciesname=S.spDD04

# Formatted dates
metadata=/BritishFormattedDates-02292024.txt
